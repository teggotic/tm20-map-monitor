...
class HasTrackmaniaComClient env where
  trackmaniaComClientL :: Lens' env ClientEnv
...
authorizeNadeo ::
  (MonadIO m, MonadReader env m, HasNadeoCoreClient env, HasNadeoThrottler env, HasNadeoRequestRate env, HasLogFunc env) =>
  Text ->
  Text ->
